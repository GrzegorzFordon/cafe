...
function Room() {
  ...
  return (
    <div className="relative flex size-full max-h-80 max-w-lg flex-col items-center justify-start gap-2 rounded bg-amber-800 p-2 font-bold text-black">
      ...
      <div className="flex w-full justify-start gap-2 rounded bg-amber-950 p-2">
        <motion.button
          ...
          onClick={() => leaveRoom({ roomID: activeRoomID })}
        >
          Leave
        </motion.button>
        <div className="rounded p-2 text-amber-200 select-none">
          {activeRoomID} ({roomData.status})
        </div>
        <div className="w-full" />
        <motion.button
          whileTap={{ scale: 0.95 }}
          className="cursor-pointer rounded bg-amber-200 p-2 hover:bg-amber-300"
          onClick={() => startGame({ roomID: activeRoomID })}
        >
          Play
        </motion.button>
      </div>
      <div className="flex size-full flex-col items-center justify-center gap-2 rounded bg-amber-950">
        ...
        {roomData.players &&
          roomData.players.map((player) => (
            <p>Player: {player.id.substring(0, 5)}</p>
          ))}
      </div>
    </div>
  );
